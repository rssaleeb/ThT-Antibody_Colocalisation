...
# -*- coding: utf-8 -*-
"""
Purpose: uses tetraspeck beads to align split channel images acquired on the ONI Nanoimager
"""
...
result = ird.similarity(greenSlice, redSlice, numiter=5)
...
for folder in folderList:
    if not folder == '.DS_Store':
        ...
        for number in range(0, FOV_Num):
                ...
                print(namePrefix + str(number) + nameSuffix + red_ID)
                
                # Load image to be transformed
                data_red = io.imread(dirPath + folder + "/" + namePrefix + str(number) + nameSuffix + red_ID).astype('uint16')
                
                # Create an empty image array matching the dimensions of the loaded image
                newresult = np.empty((data_red.shape[0], data_red.shape[1], data_red.shape[2])).astype('uint16')
                ...
                for i in range(data_red.shape[0]):
                    newresult[i]=ird.transform_img_dict(data_red[i], result, bgval=None, order=1, invert=False).astype('uint16')
                
                # Save the translated output
                imageio.mimwrite(dirPath + folder + "/" + namePrefix + str(number) + nameSuffix + red_ID[0:red_ID.index(".tif")] + '-trans.tif', newresult)
